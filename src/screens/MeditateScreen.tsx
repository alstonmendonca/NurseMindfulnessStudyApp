...
import { getMeditationsByCategory, MediaItem } from '../data/mediaContent';
import { BackgroundDoodles } from '../components/BackgroundDoodles';

type Props = NativeStackScreenProps<MainStackParamList, 'Meditate'>;

export const MeditateScreen: React.FC<Props> = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* Meditation Sections by Category */}
        {categories.map((category) => (
          <View key={category} style={styles.section}>
            <Text style={styles.sectionTitle}>{category}</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
              {meditationsByCategory[category].map((item) => (
                <TouchableOpacity 
                  key={item.id} 
                  style={styles.card} 
                  onPress={() => handlePlayAudio(item, meditationsByCategory[category])}
                >
